...
function Invoke-CapturedProcess {
  ...
  return @{
    ExitCode = $exitCode
    Output = (($output | ForEach-Object { $_.ToString() }) | Out-String)
  }
}
...
$testRunnerPath = Join-Path $PSScriptRoot "run_tests.py"
$runArguments = @($runtime.Arguments) + @($testRunnerPath)
$run = Invoke-CapturedProcess -FilePath $runtime.Path -Arguments $runArguments
