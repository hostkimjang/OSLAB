...
if (Test-Path -LiteralPath $portablePython) {
  $runtime = Test-PythonRuntime -Path $portablePython
}
...
$scriptPath = Join-Path $PSScriptRoot "hello.py"
...
if ($exitCode -ne 0 -and -not [string]::IsNullOrWhiteSpace($result.stdout)) {
